...
export async function buildTwelveInchPdf(
  ...
  const title = 'Cratedb — 12-inch singles';
  page.drawText(title, {
    x: MARGIN,
    y,
    size: TITLE_SIZE,
  ...
  if (meta.capped) {
    page.drawText(
      truncate(
        `Note: Only the first ${rows.length.toLocaleString()} of ${meta.totalInDatabase.toLocaleString()} rows are included (export limit).`,
        95
      ),
      {
        x: MARGIN,
