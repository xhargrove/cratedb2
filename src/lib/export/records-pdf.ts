...
export async function buildRecordsPdf(
  ...
  const title = 'Cratedb — Collection (albums)';
  page.drawText(title, {
    x: MARGIN,
    y,
    size: TITLE_SIZE,
  ...
  if (meta.capped) {
    page.drawText(
      truncate(
        `Note: Only the first ${rows.length.toLocaleString()} of ${meta.totalInDatabase.toLocaleString()} records are included (export limit).`,
        95
      ),
      {
        x: MARGIN,
  ...
  const columnHint =
    'Artist · Album title · Year · Genre · Storage — notes (truncated for PDF)';
